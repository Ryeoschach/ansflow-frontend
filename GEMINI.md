...
### 2.5 AI 与 实时追踪规范
- **打字机动效：** 所有流式对话（Streaming）必须集成 `ai-typing` 动画，确保 AI 思考过程具有视觉律动。
- **状态角标：** 全局 AI 助手必须提供明确的状态反馈（分析中/完成/超时/错误），避免静默等待。
- **进度可视化：** 自愈或异步触发的任务必须提供 `Progress` 进度条，并优先支持详情页面的联动跳转。
- **闭环按钮：** 重要的 AI 诊断结论底部必须集成“存入知识库”操作，并实现即时状态同步（已存入不可重复点击）。
