...
const PipelineWebhooks: React.FC = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            key: 'secret_key',
            width: 100,
            render: (hasKey: string) => hasKey ? <Tag icon={<KeyOutlined />} color="success">{t('webhook.configured')}</Tag> : <Tag color="warning">{t('webhook.notConfigured')}</Tag>,
        },
        {
                    ...
                    <Form.Item
                        label={
                            <Space>
                                <KeyOutlined />
                                {t('webhook.secretKey')}
                                {!editingWebhook && <Tag color="processing">{t('webhook.autoGenerated')}</Tag>}
                            </Space>
                        }
                        name="secret_key"
                        ...
                        {detailWebhook.secret_key && (
                            <Card size="small" title={<Space><KeyOutlined /> {t('webhook.secretKey')}</Space>}>
                                <div className="flex items-center gap-2 mb-2">
                                    <Tag color="success">{t('webhook.configured')}</Tag>
